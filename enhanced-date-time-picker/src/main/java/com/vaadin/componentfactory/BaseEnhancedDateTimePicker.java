...
class EnhancedDateTimePickerDatePicker extends com.vaadin.flow.component.datepicker.DatePicker {
  @Override
  ...
  void passThroughPresentationValue(LocalDate newPresentationValue) {
    ...
    if (valueEquals(newPresentationValue, getEmptyValue()) && isInputValuePresent()) {
...
class EnhancedDateTimePickerTimePicker extends com.vaadin.componentfactory.EnhancedTimePicker {
  @Override
  protected void validate() {
    // Should not change invalid state
  }
...
/**
 * Date Time Picker is an input field for selecting both a date and a time. The date and time can be
 * entered directly using a keyboard in the format of the current locale or through the Date Time
 * Picker’s two overlays. The overlays open when their respective fields are clicked or any input is
 * entered when the fields are focused.
 *
 * @author Vaadin Ltd
 */
@Tag("vaadin-date-time-picker")
@Uses(DateTimePicker.class)
public class BaseEnhancedDateTimePicker
    extends AbstractSinglePropertyField<BaseEnhancedDateTimePicker, LocalDateTime>
    implements Focusable<BaseEnhancedDateTimePicker>, HasAutoOpen, HasClientValidation, HasHelper,
    InputField<AbstractField.ComponentValueChangeEvent<BaseEnhancedDateTimePicker, LocalDateTime>, LocalDateTime>,
    HasOverlayClassName, HasThemeVariant<DateTimePickerVariant>, HasValidationProperties,
    HasValidator<LocalDateTime> {

  private final EnhancedDateTimePickerDatePicker datePicker =
      new EnhancedDateTimePickerDatePicker();
  private final EnhancedDateTimePickerTimePicker timePicker =
      new EnhancedDateTimePickerTimePicker();
   ...
   * Default constructor.
   */
  public BaseEnhancedDateTimePicker() {
   ...
   * @see #setLabel(String)
   */
  public BaseEnhancedDateTimePicker(String label) {
    this();
    setLabel(label);
  }
   ...
   * @see #setLabel(String)
   */
  public BaseEnhancedDateTimePicker(String label, LocalDateTime initialDateTime) {
   ...
   * @param initialDateTime the pre-selected date time in the picker
   */
  public BaseEnhancedDateTimePicker(LocalDateTime initialDateTime) {
   ...
   */
  public BaseEnhancedDateTimePicker(LocalDateTime initialDateTime, Locale locale) {
    this(initialDateTime);
    setLocale(locale);
  }

  /**
   * Sets the selected date and time value of the component. The value can be cleared by setting
   * null.
   *
   * <p>
   * The value will be truncated to millisecond precision, as that is the maximum that the time
   * picker supports. This means that {@link #getValue()} might return a different value than what
   * was passed in.
   *
   * @param value the LocalDateTime instance representing the selected date and time, or null
   */
  @Override
  public void setValue(LocalDateTime value) {
    ...
    boolean isInputValuePresent =
        timePicker.isInputValuePresent() || datePicker.isInputValuePresent();
    boolean isValueRemainedEmpty =
        valueEquals(oldValue, getEmptyValue()) && valueEquals(value, getEmptyValue());
   ...
   *
   * <p>
   * Truncates value to millisecond precision, as that is the maximum that the time picker supports.
   * This is also necessary to synchronize with the internal value of the Flow TimePicker, which
   * truncates the value as well.
   *
   * @param value the LocalDateTime instance to sanitize, can be null
   ...
   *
   * @param ariaLabel the value to set as aria-label
   */
   ...
   *
   * @return an optional aria-label or an empty optional if no aria-label has been set
   */
   ...
   * <p>
   * The final value is a concatenation of the accessible label from DateTimePicker's
   * {@link #getAriaLabel()} or {@link #getLabel()} and the given accessible label.
   *
   * @param dateLabel the value to be used as part of date picker aria-label.
   */
  public void setDateAriaLabel(String dateLabel) {
    ...
    getElement().setPropertyJson("i18n", JsonSerializer.toJson(dateTimePickerI18n));
  }

  /**
   * Gets the accessible label of the date picker.
   * <p>
   * Note that this method will return the last value passed to {@link #setDateAriaLabel(String)},
   * not the value currently set on the `aria-label` attribute of the date picker input element.
   *
   ...
   * {@link #getAriaLabel()} or {@link #getLabel()} and the given accessible label.
   *
   * @param timeLabel the value to be used as part of time picker aria-label.
   */
   ...
   * <p>
   * Note that this method will return the last value passed to {@link #setTimeAriaLabel(String)},
   * not the value currently set on the `aria-label` attribute of the time picker input element.
   *
   * @return an optional label or an empty optional if no label has been set
   */
  ...
  /**
   * Sets the {@code step} property of the time picker using duration. It specifies the intervals
   * for the displayed items in the time picker dropdown and also the displayed time format.
   * <p>
   * The set step needs to evenly divide a day or an hour and has to be larger than 0 milliseconds.
   * By default, the format is {@code hh:mm} (same as * {@code Duration.ofHours(1)}
   * <p>
   * If the step is less than 60 seconds, the format will be changed to {@code hh:mm:ss} and it can
   * be in {@code hh:mm:ss.fff} format, when the step is less than 1 second.
   * <p>
   ...
   * {@link com.vaadin.flow.component.HasValue.ValueChangeEvent} to be fired if some parts (eg.
   * seconds) is discarded from the value.</em>
   *
   * @param step the step to set, not {@code null} and should divide a day or an hour evenly
   */
  public void setStep(Duration step) {
    Objects.requireNonNull(step, "Step cannot be null");

    getElement().setProperty("step", StepsUtil.convertDurationToStepsValue(step));
  ...
  /**
   * Set the Locale for the DateTimePicker. The displayed date and time will be matched to the
   * format used in that locale.
   *
  ...
  /**
   * Synchronizes the theme attribute to the internal DatePicker and TimePicker components.
   */
   ...
   * @return <code>true</code> if the theme name was removed, <code>false</code> if the theme list
   *         didn't contain the theme name
   */
  @Override
   ...
   *
   * @param themeName a space-separated string of theme names to set, or empty string to remove all
   *        theme names
   */
  @Override
   ...
   * @param set <code>true</code> to set the theme name, <code>false</code> to remove it
   */
  @Override
  ...
  /**
   * Adds one or more theme names to this component. Multiple theme names can be specified by using
   * multiple parameters.
   *
  ...
  /**
   * Removes one or more theme names from component. Multiple theme names can be specified by using
   * multiple parameters.
   *
  ...
  public Registration addValidationStatusChangeListener(
    ...
    return addClientValidatedEventListener(event -> listener
        .validationStatusChanged(new ValidationStatusChangeEvent<>(this, event.isValid())));
  }

  private ValidationResult checkValidity(LocalDateTime value) {
    boolean hasNonParsableDatePickerValue =
        datePicker.getValue() == datePicker.getEmptyValue() && datePicker.isInputValuePresent();

    boolean hasNonParsableTimePickerValue =
        timePicker.getValue() == timePicker.getEmptyValue() && timePicker.isInputValuePresent();
  ...
  private boolean isInvalid(LocalDateTime value) {
    var requiredValidation = ValidationUtil.checkRequired(required, value, getEmptyValue());
   ...
   *
   * @param min the minimum date and time that is allowed to be set, or <code>null</code> to remove
   *        any minimum constraints
   */
   ...
   * disabled in the popups.
   *
   * @return the minimum date and time that is allowed to be set, or <code>null</code> if there's no
   *         minimum
   */
   ...
   * disabled in the popups.
   *
   * @param max the maximum date and time that is allowed to be set, or <code>null</code> to remove
   *        any minimum constraints
   */
   ...
   * disabled in the popups.
   *
   * @return the maximum date and time that is allowed to be set, or <code>null</code> if there's no
   *         minimum
   */
   ...
   * the component if not set back using {@link DateTimePicker#setDatePickerI18n(DatePickerI18n)}
   *
   ...
   *
   * @param requiredIndicatorVisible the value of the requiredIndicatorVisible to be set
   */
  @Override
  ...
  public EnhancedDateTimePickerDatePicker getDatePicker() {
    return datePicker;
  }

  public EnhancedDateTimePickerTimePicker getTimePicker() {
    return timePicker;
  }
